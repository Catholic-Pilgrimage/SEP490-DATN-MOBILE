...
interface GuestLoginModalProps {
  ...
  message?: string;
  /**
   * Optional async callback fired BEFORE navigating to login.
   * Use this to persist context (e.g. save a pending invite token)
   * so it can be restored after the user logs in.
   */
...
export const GuestLoginModal: React.FC<GuestLoginModalProps> = ({
  ...
  const handleLogin = async () => {
    ...
    setTimeout(async () => {
      // Allow caller to persist context before we reset navigation
      if (onBeforeLogin) {
        await onBeforeLogin();
      }
      await exitGuestMode();
