    ...
    onError: (_err, _newVal, context) => {
      ...
      if (context?.previousDetail !== undefined) {
        queryClient.setQueryData(
          postKeys.detail(_newVal.postId),
  ...
  return useMutation({
    ...
    onMutate: async () => {
      ...
      queryClient.setQueryData(postKeys.detail(postId), (old: any) => {
        ...
        const actualPost = old.data || old;
        const currentCount =
          actualPost.comment_count || actualPost.comments_count || 0;
        const newPost = {
          ...actualPost,
          comment_count: currentCount + 1,
          comments_count: currentCount + 1,
    ...
    onSuccess: (newCommentResponse) => {
      const newComment =
        newCommentResponse?.data || (newCommentResponse as any);
              ...
              if (index === 0) {
                if (page.data?.items) {
                  return {
                    ...
                    data: {
                      ...page.data,
                      items: [newComment, ...page.data.items],
                    },
                  };
                }
                if (page.items) {
...
export const useTranslateComment = (postId: string) => {
  return useMutation({
    mutationFn: (commentId: string) =>
      postApi.translateComment(postId, commentId),
  });
};
