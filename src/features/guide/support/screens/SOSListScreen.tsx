...
type RootStackParamList = {
  SOSDetail: { id: string; autoOpenMap?: boolean };
};

type SOSListScreenNavigationProp =
...
const SOSItem = ({
  ...
  const getStatusLabel = (status: SOSStatus) => {
    switch (status) {
      case "pending":
        return t("sos.guideList.status.pending");
      case "accepted":
        return t("sos.guideList.status.accepted");
      case "resolved":
        return t("sos.guideList.status.resolved");
      case "cancelled":
        return t("sos.guideList.status.cancelled");
      default:
        return t("sos.statusUnknown");
    }
  };
        ...
        <View style={styles.itemHeaderLeft}>
          <View style={styles.avatarContainer}>
            <Image
              source={{
                uri:
                  item.pilgrim?.avatar_url ||
                  "https://ui-avatars.com/api/?name=" +
                    (item.pilgrim?.full_name || "User"),
              }}
              style={styles.avatar}
            />
          </View>
          <TouchableOpacity onPress={onPress}>
            <Text style={styles.pilgrimName}>
              {item.pilgrim?.full_name || t("sos.guideList.defaultPilgrimName")}
            </Text>
      ...
      <TouchableOpacity style={styles.messageContainer} onPress={onPress}>
        ...
        <View style={styles.messageContent}>
          <Text style={styles.itemMessage} numberOfLines={2}>
            {item.message || t("sos.guideList.defaultMessage")}
          </Text>
      ...
      <View style={styles.itemFooter}>
        ...
        <View style={styles.locationInfo}>
          <Text style={styles.locationText} numberOfLines={1}>
            {t("sos.guideList.locationAt", {
              site: item.site?.name || t("sos.guideList.defaultArea"),
            })}
          </Text>
          <TouchableOpacity style={styles.quickMapButton} onPress={onMapPress}>
            <Ionicons name="navigate" size={14} color={GUIDE_COLORS.primary} />
            <Text style={styles.quickMapText}>
              {t("sos.guideList.navigate")}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
...
export const SOSListScreen = () => {
  ...
  const showMissingPhoneDialog = async () => {
    await confirm({
      ...
      title: t("sos.guideList.missingPhone.title"),
      message: t("sos.guideList.missingPhone.message"),
      confirmText: t("sos.guideList.missingPhone.confirm"),
      showCancel: false,
    });
  };
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>{t("sos.guideList.title")}</Text>
      ...
      <View style={styles.tabsContainer}>
        <FilterTab
          label={t("sos.guideList.tabs.pending")}
          active={activeTab === "pending"}
          onPress={() => setActiveTab("pending")}
        />
        <FilterTab
          label={t("sos.guideList.tabs.accepted")}
          active={activeTab === "accepted"}
          onPress={() => setActiveTab("accepted")}
        />
        <FilterTab
          label={t("sos.guideList.tabs.history")}
      ...
      {isLoading && !isRefetching ? (
        ...
        <FlatList
          ...
          contentContainerStyle={[
            styles.listContent,
            { paddingBottom: insets.bottom + GUIDE_SPACING.lg },
          ]}
          refreshControl={
            <RefreshControl
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              />
              <Text style={styles.emptyText}>{t("sos.guideList.empty")}</Text>
            </View>
          }
        />
      )}
