...
/** Prefill màn CreatePlanScreen (tab Lịch trình) khi sao chép hành trình từ bài đăng. */
function buildCreatePlanCloneParamsFromJourney(
  journey: { id: string; name?: string; start_date?: string; end_date?: string; number_of_people?: number; transportation?: string; member_deposit?: number; deposit_amount?: number; penalty_percentage?: number },
  tf: TFunction,
) {
  const t0 = tomorrowYMD();
  let duration = 3;
  if (journey.start_date && journey.end_date) {
    const d1 = parseYMDLocal(journey.start_date).getTime();
    const d2 = parseYMDLocal(journey.end_date).getTime();
    duration = Math.max(1, Math.ceil((d2 - d1) / (1000 * 60 * 60 * 24)));
  }
  const endYmd = addDaysToYMD(t0, duration);
  let transport = String(journey.transportation || "bus");
  if (transport === "motorcycle") transport = "motorbike";
  if (transport !== "bus" && transport !== "car" && transport !== "motorbike") {
    transport = "bus";
  }
  const dep = journey.member_deposit ?? journey.deposit_amount ?? 0;
  const pen = journey.penalty_percentage ?? 0;
  return {
    cloneSourcePlanId: journey.id,
    prefillName: `${journey.name ?? ""} (${tf("common.copy", { defaultValue: "Bản sao" })})`,
    prefillStartDate: t0,
    prefillEndDate: endYmd,
    prefillPeople: Math.max(1, Number(journey.number_of_people) || 1),
    prefillTransportation: transport,
    prefillDeposit: Number(dep) || 0,
    prefillPenalty: Number(pen) || 0,
  };
}
        ...
        {journey.id && !isGuide && (
          ...
          >
            <MaterialIcons
              ...
              color={COLORS.white}
            />
            <Text style={styles.cloneBtnText}>{t("planner.cloneJourney")}</Text>
          </TouchableOpacity>
        )}
      </View>
    </View>
