...
const FILTERS = [
  ...
  },
  {
    ...
    hasEvents: "true",
    openNow: false,
    labelKey: "allSites.filterEvents",
  },
] as const;
...
export const AllSitesScreen: React.FC<Props> = ({ navigation }) => {
  ...
  const buildSiteFilters = (
    filterKey: (typeof FILTERS)[number]["key"],
    queryValue: string,
  ) => {
    ...
    return {
      ...
      has_events: selectedFilter?.hasEvents,
    };
  };
  ...
  const handleFavoriteToggle = (siteId: string) => {
    if (!isAuthenticated || isGuest) {
      Alert.alert(
        t("allSites.loginRequiredTitle", { defaultValue: "Yêu cầu đăng nhập" }),
        t("allSites.loginRequiredMessage", {
          defaultValue: "Vui lòng đăng nhập để lưu địa điểm yêu thích.",
        }),
        [
          {
            text: t("allSites.loginLater", { defaultValue: "Để sau" }),
            style: "cancel",
          },
          {
            text: t("allSites.loginNow", { defaultValue: "Đăng nhập" }),
            onPress: () =>
              navigation.dispatch(
                CommonActions.reset({
                  index: 0,
  ...
  const handleSearch = () => {
    ...
    fetchSites(buildSiteFilters(activeFilter, normalizedQuery));
  };
  ...
  const handleClearSearch = () => {
    ...
    setSearchQuery("");
    setActiveFilter("all");
    fetchSites(buildSiteFilters("all", ""));
  };

  const handleFilterPress = (filterKey: (typeof FILTERS)[number]["key"]) => {
    setActiveFilter(filterKey);
    fetchSites(buildSiteFilters(filterKey, searchQuery));
  };
      ...
      <SafeAreaView style={styles.safeArea}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.headerTitle}>
            {t("allSites.title", { defaultValue: "Tất cả điểm đến" })}
          </Text>
          ...
          <View style={styles.searchInputWrapper}>
            ...
            <TextInput
              ...
              placeholder={t("allSites.searchPlaceholder", {
                defaultValue: "Tìm kiếm địa điểm...",
              })}
          ...
          <View style={styles.centerContainer}>
            ...
            >
              <Text style={styles.retryButtonText}>
                {t("allSites.retry", { defaultValue: "Thử lại" })}
              </Text>
            </TouchableOpacity>
          </View>
        ) : (
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <VerticalSiteCard
                  ...
                  onPress={() =>
                    navigation.navigate("SiteDetail", { siteId: item.id })
                  }
                ...
                !isLoading ? (
                  <View style={styles.emptyContainer}>
                    ...
                    <Text style={styles.emptyText}>
                      {searchQuery
                        ? t("allSites.emptySearch", {
                            query: searchQuery,
                            defaultValue: `Không tìm thấy "${searchQuery}"`,
                          })
                        : t("allSites.empty", {
