...
export const PlanCard: React.FC<PlanCardProps> = ({ plan, onPress }) => {
  ...
  const translateStatus = (status?: string): string => {
    switch (status?.toLowerCase()) {
      case "planning":
        return t("planner.statusPlanning");
      case "planned":
        return t("planner.statusPlanned");
      case "ongoing":
        return t("planner.statusOngoing");
      case "completed":
        return t("planner.statusCompleted");
      case "cancelled":
        return t("planner.statusCancelled");
      case "draft":
        return t("planner.statusDraft");
      default:
        return status || t("planner.statusPlanning");
    }
  };
        ...
        {plan.isShared && (
          ...
          >
            <Ionicons
              ...
              style={{ marginRight: 4 }}
            />
            <Text style={[styles.statusText, { color: "#FA8C16" }]}>
              {t("planner.shared")}
            </Text>
          </View>
        )}
    ...
    <TouchableOpacity style={styles.card} onPress={onPress} activeOpacity={0.9}>
      ...
      <View style={styles.leftColumn}>
        ...
        <View style={styles.dayBadge}>
          ...
          <Text style={styles.dayBadgeText}>{t("planner.days")}</Text>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  placeholderOverlay: {
    ...
    backgroundColor:
      "linear-gradient(180deg, rgba(255,255,255,0.1) 0%, rgba(0,0,0,0.1) 100%)", // Simulated
  },
  dayBadge: {
    position: "absolute",
    top: 8,
