...
import type { MapPin, VietmapViewRef } from "../../../../components/map/VietmapView";
...
const DAY_COLORS = [
  "#D35400", "#1A6B8A", "#6B3A8A", "#1A7A4A",
  "#8A3A1A", "#1A3A8A", "#8A6B1A", "#5C1A8A",
];
const STATUS_COLORS = { visited: "#22C55E", skipped: "#9CA3AF", next: "#DC2626" };
...
export default function PlannerMapScreen({ route, navigation }: Props) {
  ...
  const { planId, focusItemId, focusDay, itemsByDay: passedItems } = route.params;
  ...
  const [liveRoute, setLiveRoute] = useState<RouteResultWithGeometry | null>(null);
  ...
  const ibd = useMemo(() => passedItems || plan?.items_by_day || {}, [passedItems, plan?.items_by_day]);
  ...
  const dayItems = useMemo(() => {
    if (!activeDay) return [];
    return (ibd[activeDay] || []).filter((i) => i.site?.latitude && i.site?.longitude);
  }, [activeDay, ibd]);
  ...
  const globalNextStop = useMemo(() => {
    for (const day of sortedDays) {
      for (const it of (ibd[day] || [])) {
        ...
        if (s !== "visited" && s !== "skipped" && it.site?.latitude && it.site?.longitude) {
          return { item: it, day };
        }
      }
    }
  ...
  const dayPins: MapPin[] = useMemo(() => {
    ...
    return dayItems.map((it, i) => {
      ...
      return {
        ...
        title: it.site.name,
        subtitle: isNext ? `Điểm ${i + 1} • Tiếp theo`
          : st === "visited" ? `Điểm ${i + 1} • Đã viếng ✓`
          : st === "skipped" ? `Điểm ${i + 1} • Bỏ qua`
          : `Điểm ${i + 1}`,
        color,
        icon: `${i + 1}`,
      };
    });
  }, [dayItems, activeDay, sortedDays, nextStopId]);
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      } catch { if (!off) setRouteData(null); }
      finally { if (!off) setRouteLoading(false); }
    })();
    return () => { off = true; };
  }, [waypoints]);
  ...
  useEffect(() => {
    if (!userLoc || !nextPin) { setLiveRoute(null); return; }
  ...
  const mapCenter = useMemo(() => {
    if (focusItemId) {
      ...
      if (fp) return { latitude: fp.latitude, longitude: fp.longitude, zoom: 12 };
    }
    const pins = dayPins.length > 0 ? dayPins : allPins;
    if (!pins.length) return { latitude: 10.762622, longitude: 106.660172, zoom: 7 };
    if (pins.length === 1)
      return { latitude: pins[0].latitude, longitude: pins[0].longitude, zoom: 13 };
  ...
  const cards: CardData[] = useMemo(() => {
    ...
    for (let i = 0; i < dayPins.length; i++) {
      ...
      const cumDur = routeData
        ? routeData.segments.slice(0, i).reduce((a, s) => a + s.route.durationMinutes, 0)
        : 0;
      const it = dayItems[i];
      const isNext = it?.id === nextStopId;
      ...
      out.push({
        key: dayPins[i].id,
        fromName: i === 0 ? "Vị trí của bạn" : dayPins[i - 1].title,
        toName: dayPins[i].title,
        distKm: useLive ? liveRoute!.distanceKm : (seg?.route.distanceKm ?? 0),
        durMin: useLive ? liveRoute!.durationMinutes : (seg?.route.durationMinutes ?? 0),
    ...
    return out;
  }, [dayPins, routeData, dayItems, nextStopId, liveRoute, activeDay, sortedDays, ibd]);

  // ─── Handlers ───
  const getDayLabel = useCallback(
    (_: string, i: number) => {
      if (!plan?.start_date) return `Ngày ${i + 1}`;
      const d = new Date(plan.start_date);
  ...
  const autoFit = useCallback(() => {
    ...
    const pts = [...dayPins.map((p) => ({ lat: p.latitude, lng: p.longitude }))];
  ...
  const getDelay = useCallback((c: CardData): string | null => {
    if (!c.estTime || c.status === "visited" || c.status === "skipped") return null;
    try {
      const [eh, em] = c.estTime.split(":").map(Number);
      const [ah, am] = c.eta.split(":").map(Number);
      const diff = (ah * 60 + am) - (eh * 60 + em);
      if (diff > 10) return `Chậm ${diff} phút so với kế hoạch`;
    } catch { /* ignore */ }
    return null;
  }, []);

  const focusSite = useMemo(
    () => (focusItemId ? dayPins.find((p) => p.id === focusItemId)?.title ?? null : null),
    [focusItemId, dayPins],
  );
  ...
  if (!plan || error) {
    return (
      <View style={[st.center, { backgroundColor: COLORS.background }]}>
        ...
        <Text style={st.errText}>{error || "Không tìm thấy hành trình"}</Text>
        <TouchableOpacity onPress={() => navigation.goBack()}>
          <Text style={st.errLink}>Quay lại</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  return (
    <View style={st.root}>
      ...
      {/* ── TOP BAR ── */}
      <View style={[st.topBar, { paddingTop: insets.top + (Platform.OS === "android" ? 12 : 8) }]}>
        <TouchableOpacity style={st.topBtn} onPress={() => navigation.goBack()}>
          <Ionicons name="arrow-back" size={22} color="#fff" />
        </TouchableOpacity>
        <View style={st.topInfo}>
          <Text style={st.topName} numberOfLines={1}>{plan.name}</Text>
          <Text style={st.topSub}>
            {dayPins.length} điểm{activeDay ? ` • Ngày ${activeDay}` : ""}
            {routeData ? ` • ${fmtDist(routeData.totalDistanceKm)} • ${fmtDur(routeData.totalDurationMinutes)}` : ""}
          </Text>
        </View>
        <TouchableOpacity style={st.topBtn} onPress={() => setSelDay(null)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          <Ionicons name="layers-outline" size={22} color={selDay === null ? COLORS.accent : "#fff"} />
        </TouchableOpacity>
      </View>
      ...
      {sortedDays.length > 1 && (
        <View style={st.dayBar}>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={st.dayBarInner}>
            {sortedDays.map((day, di) => {
              ...
              return (
                <TouchableOpacity
                  key={day}
                  style={[st.dayPill, on && { backgroundColor: dc, borderColor: dc }]}
                  onPress={() => { setSelDay(day); setActiveIdx(0); }}
                  activeOpacity={0.75}
                >
                  {di === todayIdx && <View style={[st.todayDot, { backgroundColor: on ? "#fff" : dc }]} />}
                  <Text style={[st.dayText, on && { color: "#fff" }]}>{getDayLabel(day, di)}</Text>
                  <Text style={[st.dayCount, on && { color: "rgba(255,255,255,0.8)" }]}>
                    {(ibd[day] || []).filter((i) => i.site?.latitude).length} điểm
                  </Text>
                </TouchableOpacity>
              );
            })}
      ...
      {focusSite && (
        <View style={st.focusBanner}>
          ...
          <Text style={st.focusBannerText} numberOfLines={1}>Xem trước: {focusSite}</Text>
          <TouchableOpacity onPress={() => navigation.setParams({ focusItemId: undefined })} style={st.focusBannerBtn}>
            <Text style={st.focusBannerBtnText}>Xem tất cả</Text>
          </TouchableOpacity>
        </View>
      )}
      ...
      {(routeLoading || liveRouteLoading) && (
        <View style={st.routeBar}>
          ...
          <Text style={st.routeBarText}>
            {liveRouteLoading ? "Đang tính đường đến điểm tiếp theo..." : "Đang tính tuyến đường..."}
          </Text>
        </View>
      )}
      ...
      {liveRoute && nextPin && !routeLoading && !liveRouteLoading && (
        <View style={st.liveBanner}>
          ...
          <Text style={st.liveBannerText}>
            Đến {nextPin.title}: {fmtDist(liveRoute.distanceKm)} • {fmtDur(liveRoute.durationMinutes)} • ETA {fmtETA(liveRoute.durationMinutes)}
          </Text>
        </View>
      )}
      ...
      <View style={st.mapWrap}>
        {dayPins.length === 0 ? (
          <View style={st.emptyMap}>
            ...
            <Text style={st.emptyText}>Ngày này chưa có điểm hành hương</Text>
          </View>
        ) : (
          <VietmapView
            ref={mapRef}
        ...
        {/* ── FABs ── */}
        {dayPins.length > 0 && (
          <View style={[st.fabCol, { bottom: insets.bottom + (cards.length > 0 ? 165 : 24) }]}>
            <TouchableOpacity style={st.fab} onPress={autoFit} activeOpacity={0.8}>
      ...
      {/* ── CAROUSEL ── */}
      {cards.length > 0 && (
        <View style={[st.carousel, { paddingBottom: Math.max(insets.bottom, 12) + 4 }]}>
          <FlatList
            data={cards}
            horizontal
            ...
            renderItem={({ item: c, index: i }) => (
              <SegCard card={c} active={i === activeIdx} delay={getDelay(c)} onPress={() => scrollToCard(i)} />
            )}
          />
        </View>
      )}
...
// ─── SegCard (memoized) ──────────────────────────────────────
const SegCard = React.memo(({ card: c, active, delay, onPress }: {
  card: CardData; active: boolean; delay: string | null; onPress: () => void;
}) => {
  ...
  return (
    <TouchableOpacity
      style={[st.card, active && st.cardOn, vis && st.cardVis, skip && st.cardSkip, preview && st.cardPreview]}
      onPress={onPress} activeOpacity={0.92}
    >
      {/* Row 1 */}
      <View style={st.cardR1}>
        <View style={[st.badge, done && { backgroundColor: vis ? STATUS_COLORS.visited : STATUS_COLORS.skipped }, preview && { backgroundColor: "#6366F1" }]}>
          {vis ? <Ionicons name="checkmark" size={14} color="#fff" />
            : preview ? <Ionicons name="eye-outline" size={14} color="#fff" />
            : <Text style={st.badgeText}>{c.idx}</Text>}
        </View>
        <View style={st.cardNameW}>
          <Text style={[st.cardName, done && { color: COLORS.textTertiary }]} numberOfLines={1}>{c.toName}</Text>
          {c.idx > 1 && !preview && <Text style={st.cardFrom} numberOfLines={1}>Từ: {c.fromName}</Text>}
          {preview && <Text style={st.cardFrom}>Xem trước ngày tiếp theo</Text>}
        </View>
        {c.isNext && !done && <View style={st.tNext}><View style={st.tDot} /><Text style={st.tNextT}>Tiếp theo</Text></View>}
        {vis && <View style={st.tVis}><Text style={st.tVisT}>Đã viếng</Text></View>}
        {skip && <View style={st.tSkip}><Text style={st.tSkipT}>Bỏ qua</Text></View>}
        {preview && <View style={st.tPreview}><Text style={st.tPreviewT}>Xem trước</Text></View>}
      </View>
      ...
      <View style={st.cardR2}>
        {c.distKm > 0 && (
          <>
            <View style={st.met}><Ionicons name="car-outline" size={13} color={COLORS.textSecondary} /><Text style={st.metV}>{fmtDist(c.distKm)}</Text></View>
            <Text style={st.metS}>•</Text>
            <View style={st.met}><Ionicons name="time-outline" size={13} color={COLORS.textSecondary} /><Text style={st.metV}>{fmtDur(c.durMin)}</Text></View>
            <Text style={st.metS}>•</Text>
          </>
        )}
        <View style={st.met}>
...
const st = StyleSheet.create({
  ...
  topBar: { backgroundColor: COLORS.holy, paddingHorizontal: 14, paddingBottom: 12, flexDirection: "row", alignItems: "center", gap: 10, ...SHADOWS.small },
  topBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: "rgba(255,255,255,0.18)", alignItems: "center", justifyContent: "center" },
  topInfo: { flex: 1 },
  topName: { color: "#fff", fontSize: 17, fontWeight: "800", letterSpacing: 0.3 },
  ...
  dayBarInner: { paddingHorizontal: SPACING.md, paddingVertical: SPACING.sm, gap: 8, flexDirection: "row" },
  dayPill: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: COLORS.border, backgroundColor: COLORS.background },
  ...
  focusBanner: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: SPACING.md, paddingVertical: 8, backgroundColor: "#FEF2F2", borderBottomWidth: 1, borderBottomColor: "#FECACA" },
  focusBannerText: { flex: 1, fontSize: 13, fontWeight: "700", color: "#991B1B" },
  focusBannerBtn: { paddingHorizontal: 10, paddingVertical: 4, borderRadius: 12, backgroundColor: "rgba(0,0,0,0.06)" },
  focusBannerBtnText: { fontSize: 12, fontWeight: "600", color: COLORS.textSecondary },

  routeBar: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, paddingVertical: 6, backgroundColor: "rgba(207,170,58,0.1)", borderBottomWidth: 1, borderBottomColor: COLORS.border },
  routeBarText: { fontSize: 12, color: COLORS.textSecondary, fontWeight: "500" },

  // Live route info banner
  liveBanner: { flexDirection: "row", alignItems: "center", gap: 8, paddingHorizontal: SPACING.md, paddingVertical: 8, backgroundColor: "#EFF6FF", borderBottomWidth: 1, borderBottomColor: "#BFDBFE" },
  liveBannerText: { flex: 1, fontSize: 12, fontWeight: "700", color: "#1D4ED8" },

  mapWrap: { flex: 1 },
  map: { flex: 1, borderRadius: 0 },
  emptyMap: { flex: 1, alignItems: "center", justifyContent: "center", gap: 12, backgroundColor: COLORS.background },
  emptyText: { color: COLORS.textTertiary, fontSize: 14, fontStyle: "italic" },

  fabCol: { position: "absolute", right: 14, gap: 10, alignItems: "center", zIndex: 20 },
  fab: { width: 42, height: 42, borderRadius: 21, backgroundColor: COLORS.white, alignItems: "center", justifyContent: "center", ...SHADOWS.medium, borderWidth: 1, borderColor: COLORS.borderLight },
  fabActive: { backgroundColor: COLORS.info, borderColor: COLORS.info },

  carousel: { backgroundColor: COLORS.white, borderTopWidth: 1, borderTopColor: COLORS.border, paddingTop: 10, ...SHADOWS.medium },

  card: { width: CARD_WIDTH, marginRight: CARD_GAP, backgroundColor: COLORS.surface0, borderRadius: 14, padding: 12, borderWidth: 1.5, borderColor: COLORS.borderLight, ...SHADOWS.small },
  cardOn: { borderColor: COLORS.holy, backgroundColor: "#FFFDF7", ...SHADOWS.medium },
  cardVis: { borderColor: "#BBF7D0", backgroundColor: "#F0FFF5", opacity: 0.85 },
  cardSkip: { borderColor: "#E5E7EB", backgroundColor: "#F9FAFB", opacity: 0.7 },
  cardPreview: { borderColor: "#C7D2FE", backgroundColor: "#EEF2FF", borderStyle: "dashed" as any },

  cardR1: { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 6 },
  badge: { width: 28, height: 28, borderRadius: 14, backgroundColor: COLORS.holy, alignItems: "center", justifyContent: "center" },
  ...
  tNext: { flexDirection: "row", alignItems: "center", gap: 4, backgroundColor: "#FEF2F2", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8 },
  tDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: "#DC2626" },
  tNextT: { fontSize: 10, fontWeight: "700", color: "#DC2626" },
  tVis: { backgroundColor: "#DCFCE7", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8 },
  tVisT: { fontSize: 10, fontWeight: "700", color: "#16A34A" },
  tSkip: { backgroundColor: "#F3F4F6", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8 },
  tSkipT: { fontSize: 10, fontWeight: "700", color: "#6B7280" },
  tPreview: { backgroundColor: "#E0E7FF", paddingHorizontal: 7, paddingVertical: 3, borderRadius: 8 },
  tPreviewT: { fontSize: 10, fontWeight: "700", color: "#4338CA" },

  cardR2: { flexDirection: "row", alignItems: "center", paddingVertical: 4, gap: 4, flexWrap: "wrap" },
  ...
  warn: { flexDirection: "row", alignItems: "center", gap: 5, backgroundColor: "#FFFBEB", paddingHorizontal: 8, paddingVertical: 5, borderRadius: 8, marginTop: 4, borderWidth: 1, borderColor: "#FDE68A" },
